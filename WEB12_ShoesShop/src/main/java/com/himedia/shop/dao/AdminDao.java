...
public class AdminDao {
	...
	public int getAllCount(String tablename, String fieldname, String key) {
		...
		// 검색어가 '부츠' -> concat의 결과 : '%부츠%' -> 부츠를 포함한 상품명검색
	...
	public void updateProduct(ProductVO pvo) {
		con = Db.getConnection();
		String sql = "update product set kind=?, useyn=?, name=?, price1=?, price2=?, price3=?, content=?, bestyn=?, image=?, savefilename=? where pseq=?";
	...
	public ArrayList<MemberVO> adminMemberList(Paging paging, String key) {
		...
		con = Db.getConnection();
		String sql = "select * from member where name like concat('%',?,'%') "
