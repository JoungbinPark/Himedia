...
<%
	...
	//서버에서 JSP파일로 전달되는 배열이나 ArrayList는 보통 request의 Attribute에 담겨옵니다. 
...
<!-- 
	items="" : 반복실행에 이용할 리스트 또는 배열을 지정(request나 session에 저장)
	var = "s" : 배열 또는 리스트에 값을 한번에 하나씩 저장할 때 사용할 변수명
 -->
...
${status.count} : 1부터 시작한 반복의 현재아이템
${status.index} : 0부터 시작한 반복의 현재아이템
-->
 	...
 	<c:forEach items="${mList }" var ="movie" varStatus="state">
 		<c:choose>
 			<c:when test="${state.first}">
 				<tr style="font-weight:bold; color:red;"><td>${state.index }</td><td>${state.count }</td><td>${movie }</td></tr>
 			</c:when>
 			<c:when test="${state.last}">
 				<tr style="font-weight:bold; color:blue;"><td>${state.index }</td><td>${state.count }</td><td>${movie }</td></tr>
 			</c:when>
 			<c:otherwise>
 				<tr style="font-weight:bold; color:black;"><td>${state.index }</td><td>${state.count }</td><td>${movie }</td></tr>
 			</c:otherwise>
 		</c:choose>
 	</c:forEach>
 </table>
...
${state.first} : 현재 반복이 마지막이면 true 리턴
 -->
