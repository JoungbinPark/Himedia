...
public class Application {
    public static void main(String[] args) {
        ...
        *    (다양한 기능을 사용하는 데 있어서 관리해야할 메세지 종류가 줄어든다)
        *    하나의 호출로 여러가지 동작을 수행할 수 있다는 측면에서 오버라이딩을 다형성이라 보기도 한다.
        *    다형성을 이해하기 쉬운 가장 좋은 예이기도 하다.
        *    하지만 이 부분은 이견이 많이 좋재하기 때문에 다형성을 이해하는 데 참고로만 사용하자.
        ...
        * 4. 결합도를 낮춰서 유지보수성을 증가시킬 수 있다. */
        ...
        /*
        Rabbit r = new Animal();        // 자식타입으로 부모인스턴스를 저장할 순 없다
        tiger t = new Animal();
        */

        /* 6. 동적 바인딩 확인 */
        /* 동적바인딩이란?
        * 컴파일 당시에는 해당 타입으로 메소드와 연결되어 있다가
        * 런타임 당시의 실제 객체가 가진 오버라이딩된 메소드로 바인딩을 바꿔 동작하는 것
        * */

        a1.cry();           //animal 자료형이더라도 오버라이드된 rabbit 인스턴스의 메소드로 작동
        a2.cry();
        /*
        a1.jump();          //원래 animal 메소드에 없던 rabbit의 인스턴스 메소드를 사용할 수 없다.
        a1.bite();
        */
        ...
        //((Tiger)a1).bite();     //ClassCastException 오류 : 토끼는 호랑이가 될 수 없다.
        ...
        * 또한 작성 여부에 따라 명시적 & 묵시적으로 구분할 수 있다.
        * */

        Animal animal1 = new Rabbit();   //up-casting의 묵시적 형변환
        Animal animal2 = (Animal) new Rabbit();     //up-casting의 명시적 형변환

        Rabbit rabbit1 = (Rabbit) animal1;      //down-casting의 명시적 형변환
        //Rabbit rabbit2 = animal2;               //down-casting은 묵시적 형변환이 되지 않는다.
