...
public class Application {
    public static void main(String[] args) {
        /* 학습목표 : 추상클래스와 추상메소드에 대해 이해할 수 있다. */
        /* 추상클래스와 추상메소드
        * 추상메소드를 0개 이상 포함하는 클래스를 추상클래스라고 한다.
        * 추상클래스는 클래스 선언부에 abstract 키워드를 명시해야한다.
        * 추상클래스로는 Instance 생성이 불가능하다.
        * 추상클래스를 사용하려면 추상클래스를 상속받는 하위클래스를 이용하여 인스턴스를 생성해야한다.
        * 이 때 추상클래스는 상위 타입으로 사용될 수 있으며 다형성을 이용할 수 있다.
        * */

        /* 추상클래스에 작성된 추상메소드는 반드시 후손이 오버라이딩해서 작성해야하며
        * 후손클래스들의 메소드들을 공통 인터페이스로의 역할을 수행할 수 있다.
        * 추상클래스에 작성한 메소드를 호출하게 되면
        * 실제 후손 타입의 인스턴스가 가지는 메소드는 다형성이 적용되어
        ...
        * ex) public abstract void method(); */
        ...
        product.staticMethod();             //나오긴 하는데 인스턴스 생성이 불필요하긴 함
        ...
        * 필수 기능을 정의하여 강제성을 부여해 개발 시 일관된 인터페이스를 제공할 수 있다. */
        ...
        /* 하지만 다른 클래스를 상속받고 있는 클래스를 작성할 시에는 추상클래스를 추가로 상속받을 수 없다.
